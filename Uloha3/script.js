...
let out90 = document.getElementById("output90")

function checkReference() {
    try {
        console.log(varia)
    } catch (err) {
        console.error(`${err.name} + ${err.message} mnjau`)
        out90.innerText = `${err.name}: ${err.message}. Mnjau`
...
function aa() {
    document.getElementById("input91").addEventListener("change", (e) => console.log(e))
}
...
function callFunction(param) {
    try {
        ...
        console.error(`${err.name} + ${err.message} mnjau`)
        document.getElementById("output91").innerText = `${err.name}: ${err.message}. Mnjau`
    }
}
